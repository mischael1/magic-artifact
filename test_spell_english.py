# Test spell recognition - English version
...
def test_spell_recognition():
    ...
    for spell in spells_info:
        print(f"* {spell['name']}")
        print(f"  ID: {spell['id']}")
        print(f"  Triggers: {', '.join(spell['trigger_words'])}")
        print()
    ...
    for phrase, expected_id in exact_matches:
        ...
        if result_id == expected_id:
            correct += 1
            print(f"OK: '{phrase}' -> {result_id}")
        else:
            print(f"FAIL: '{phrase}' -> {result_id} (expected {expected_id})")
